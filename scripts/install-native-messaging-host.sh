...
#
# ブラウザの設定を書き換えるので、**利用者が自分で実行する**。Genie は自動で入れない。
...
APP="${ASTRA_APP:-$ROOT/apps/genie-macos/.build/Genie.app}"
BIN="$APP/Contents/MacOS/GenieMac"
[[ -x "$BIN" ]] || { echo "Genie.app が見つからない: $BIN（先に scripts/package-macos-app.sh）" >&2; exit 1; }
...
cat > "$DIR/com.astra.desktop.context.json" <<JSON
{
  ...
  "description": "Genie Context Bridge",
  "path": "$WRAPPER",
  "type": "stdio",
