...
for t in screenshot waveform livemic livemeeting livescreen sttrecognize sttstream guishot axtree navtitle recoveryui focus breakpoints dictation state presence perf storage meetingiq vad browser dockanim entry secret recordbutton session uiscale acceptance sessionsync; do
  OUT="$("$BIN" --selftest "$t")"
  echo "$OUT"
