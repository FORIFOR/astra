...
#
#   PC A（この Mac）: Genie RC が Meet を検出 → 録音 → Notes / Captions / Ask / Pause / Resume / Stop → Library → 出所
...
#
#   bash scripts/reality/run-real-meeting.sh [Genie.app] [out-dir]
#
# 段階（揃っているものだけで、できる所まで回し、足りない段は AUTOMATION_MISSING と言う）:
#   simulate : 音を使わず台本を確定行として入れる → 判定器の配線だけ確かめる（gate ではない）
#   audio    : 実マイク経路。台本 WAV を BlackHole へ流し、既定入力を BlackHole にして Genie が本当に聞く
...
mkdir -p "$OUT"
[[ -x "$APP/Contents/MacOS/GenieMac" ]] || { echo "AUTOMATION_MISSING: 署名済み .app（scripts/package-macos-app.sh）"; exit 2; }
pgrep -x GenieMac >/dev/null && { echo "FAIL: GenieMac が既に動いている"; exit 1; }
...
case "$mode" in
  simulate)
    ...
    ;;
  audio)
    # Genie の入力を BlackHole に、再生の出力も BlackHole に。終わったら戻す。
    ...
    # 台本の進みは cue（pause / resume / stop）で Genie に伝え、Genie の state（recording / paused）を待つ。
    ...
    wait_state() {  # $1 = recording | paused
      ...
      while [[ "$(cat "$STATE" 2>/dev/null)" != "$1" ]]; do
        sleep 0.5; n=$((n+1)); [[ $n -gt 80 ]] && { echo "FAIL: Genie が $1 にならない"; return 1; }
      done
    ...
    sleep 1
    echo stop > "$CUE"
    while pgrep -x GenieMac >/dev/null; do sleep 1; done
    ;;
  meet)
...
[[ -f "$OUT/astra/result.json" ]] || { echo "FAIL: Genie が result.json を書かなかった"; exit 1; }
