...
#
#   ASTRA_TCC_CONFIRM=1 bash scripts/reality/run-live-tcc.sh [Genie.app] [out-dir]
#
...
#   deny recovery                    = PASS 断られたあと Genie が理由と「設定を開く」を出す（結果面 result-openSettings）
...
pkill -x GenieMac 2>/dev/null || true; sleep 1
...
sleep 3; pkill -x GenieMac 2>/dev/null || true
echo "  microphone dialog at feature use = $purpose  deny=$denied"
# 拒否のあと録音を始めると、結果面が理由と「設定を開く」を出すか（recoveryui selftest が state で言う）。
recov="$(ASTRA_DATA_ROOT="$data" "$APP/Contents/MacOS/GenieMac" --selftest recoveryui 2>/dev/null | tail -1)"
