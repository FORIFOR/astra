...
CARGO_CORE="$(grep -m1 '^version' "$ROOT/core/genie-core/Cargo.toml" | sed 's/.*"\(.*\)".*/\1/')"

echo "  package.json=$PKG tauri.conf.json=$TAURI src-tauri/Cargo.toml=$CARGO_APP genie-core/Cargo.toml=$CARGO_CORE"
for v in "$TAURI" "$CARGO_APP" "$CARGO_CORE"; do
  [[ "$v" == "$PKG" ]] || { echo "  版番号がずれている（$PKG と $v）" >&2; fail=1; }
done

# 配布物は**外の dylib を掴まない**こと。掴むと、その絶対パスが無い機械では起動しない。
BIN="$ROOT/apps/genie-macos/.build/release/GenieMac"
if [[ -f "$BIN" ]]; then
  if otool -L "$BIN" | grep -q "genie_core.*dylib"; then
    echo "  release の実行体が genie_core を dylib で掴んでいる" >&2; fail=1
  fi
fi
...
if grep -rn "/Users/[a-z]" "$ROOT/apps/genie-macos/Sources/GenieMac" --include='*.swift' \
     | grep -v "^.*SelfTest.swift" | grep -q .; then
  echo "  ソースに個人の絶対パスが残っている:" >&2
  grep -rn "/Users/[a-z]" "$ROOT/apps/genie-macos/Sources/GenieMac" --include='*.swift' \
    | grep -v "SelfTest.swift" | head -5 >&2
  fail=1
fi

# 組み上がった .app があるなら、**バンドルとして**成立しているか見る。
APP="$ROOT/dist/Genie.app"
if [[ -d "$APP" ]]; then
  n="$(find "$APP/Contents/Resources/plugins/builtin" -name plugin.yaml 2>/dev/null | wc -l | tr -d ' ')"
  [[ "$n" -gt 0 ]] || { echo "  dist/Genie.app に同梱プラグインが入っていない" >&2; fail=1; }
fi
