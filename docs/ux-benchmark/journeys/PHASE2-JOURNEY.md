...
## Home の入口の優先（2026-09-03、盲検 3 名。`home-intent/panel1`）

Reference: v0.1.1 の Home（golden `06-main-home`）。左に nav 7 語、右に「こんにちは」→
「何を終わらせますか？」の入力欄 → 「録音を始める」のカード → 録りかけの案内。
Hypothesis: 録音カードの太字と赤い点が、汎用の入口（入力欄）より先に読まれ、
「新しい仕事を始める場所」が入力欄だと分からない。
Measured: 盲検 3 名（j1 opus / j2 sonnet / j3 haiku）。画像だけ、観察を先に訊き、
「この画面で新しい仕事を始める場所を 1 つ」「会議を録音したいならどこ」「左の一覧」
「どちらが先に目に入るか」「主 / 同じ段 / 従」。fixture は 2 枚を手描き
（F1 = 入力欄が主・録音は下の細い link、F2 = 入力欄が無く録音カードだけ）で、
F1/F2 の答が鍵と外れた判定者は捨てる。
Candidates: A = 今のまま / B = nav の視覚的な重さを下げるだけ / C = B + 入力欄を 1 段目、
録音の入口を 2 段目。カードの追加や Home の作り直しは候補に入れない。
Gate: Evidence B（盲検の発見）で欠陥が出たときだけ B / C を作る。
success = 3 名とも「新しい仕事を始める場所」に入力欄を指す。

### fixture validation

3 名とも通った。F1: Q1 = 「ここに頼みごとを書く」、Q2 = 「会議を録る」、Q5 = 主。
F2: Q1 = 「録音を始める」（j1, j2。「他に入口が見当たらない」と添えて）または
「見当たらない」（j3）。入力欄を捏造した判定者は 0。
j3 は A の O1 で「読きから」「保存」と 1〜2 語を読み違えている（画面は「続きから」「破棄」）。
fixture は正しく、問いの答は画面の語をそのまま引いているので判定は採り、読みの粗さは記す。

### 層 B（A = 今）

| 問い                           | j1                                                       | j2             | j3                             | 数      |
| ------------------------------ | -------------------------------------------------------- | -------------- | ------------------------------ | ------- |
| Q1 新しい仕事を始める場所      | 「何を終わらせますか？」                                 | 同             | 同                             | **3/3** |
| Q2 会議を録音したい            | 「録音を始める」                                         | 同             | 同                             | **3/3** |
| Q3 左の一覧（nav の発見）      | 7 語                                                     | 5 語           | 6 語                           | 3/3     |
| Q5 Q1 は Q2 より上の段（主）か | 主                                                       | 主             | 主                             | **3/3** |
| Q4 どちらが先に目に入るか      | 判断できない（入力欄が上だが、録音の太字と赤い点が引く） | Q1             | Q2                             | 1/1/1   |
| O2 最初に目が行く              | 「こんにちは」→ 録音の赤い丸                             | 「こんにちは」 | 赤いアイコンの「録音を始める」 | —       |

**発見は 3/3 で入力欄。録音も 3/3。nav も 3/3。** 「入力欄が隠れている」という仮説は
Evidence B で支持されない。**HOME_INTENT_PRIORITY: HYPOTHESIS → NOT_A_DEFECT（A を維持）**。
B / C は作らない（「測定で欠陥が確認されるまで UI 候補を作らない」）。占有・要素数・
Craft Freeze（字の太さ）に触れるものは何も変えていない。

記録のみ（層 C）: **最初の視線だけが割れる**。1 名は入力欄、1 名は録音カード（赤い点）、
1 名は決められない。発見の結果は変わらないので判定にしない。次に測るなら、
この 3 名で C（録音の入口を 2 段目）を A/B にかけ、Q4 が 3/3 で Q1 に寄るか・
Q2 の 3/3 が落ちないかを見る。今回は Q1 が既に 3/3 なので、上がる余地が無い指標の
ために造形を変えることはしない。
