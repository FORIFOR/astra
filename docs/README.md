...
ユーザーが示した「目的から、収益やインパクトにつながる成果物を自動で作る」という方向と、最初に動画制作を通す受け入れ条件を[目的から成果物を作る実行基盤](strategy/outcome-execution.md)に記録した。[最初の動画企画草案](strategy/first-video-brief.md)も参照する。これは最新の要件と実装計画であり、下記の凍結済み仕様に記載された機能や新しい目標が、実装・実サービス検証済みであるという意味ではない。
