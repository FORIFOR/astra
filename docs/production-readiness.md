...
- **GUI の手動確認。**assistant はマウスで画面を操作していない。
  ...
  **実機でも確認した**（検証用アカウント ui-check で AX / CGEvent から操作）:
  ...
  Appendix A の部品は `#/gallery?only=…`（開発ビルド限定の見本帳、作り物のデータ）で
  状態ごとに撮り、仕様の図と突き合わせた（Work / Context Lens / 承認 / 根拠 / 会議 / 議事録 / Apps / Library / Home）。
...
- ~~手元 STT の実模型~~ → **この端末で実測した**（`scripts/install-local-stt.sh` で
  模型と dylib を入れ、`cargo test -- --ignored` 3 件 PASS。14 秒の録音 → 246ms で正解）
...
| §23 の目盛り（Dock summon < 120 ms 等）を **測る場所が画面側に無かった**。目標だけが文書にあった                                                                    | 「測っていないものは達成と言えない」— 設定に計測表を置き、Rust と runtime で測るようにした  |
