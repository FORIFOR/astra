# Genie Desktop Implementation Specification v0.1 — 適合状況

正本: `Genie Desktop Implementation Specification v0.1`（1167 行）
...
| §         | 要求                                                                                              | 実装                                                                                                                                                                                                        | ゲート                                                  |
| --------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| 5 / 31    | 状態を 1 つの State Machine に。UI ごとに状態を持たせない                                         | `Core/GenieStateStore` が唯一の置き場。`VoiceHUDState.mode` は Store への窓口（`typealias Mode = DockPresentation`）で、**二重に持たない**                                                                  | `state`                                                 |
| 5         | `GenieMode` / `GenieState`                                                                        | `Core/GenieModels`                                                                                                                                                                                          | `state`                                                 |
| 28        | `GenieEventBus`、Module 間を直接依存させない                                                      | `Core/GenieEventBus`。§28 のイベント名をそのまま持つ                                                                                                                                                        | `state`                                                 |
...
| 6         | Presence（frontmostApplication 監視）                                                             | `GenieAppDelegate` の `didActivateApplication` → `app.changed` + 文脈更新                                                                                                                                   | `presence`                                              |
...
| 15        | AgentTask / Step の Timeline UI（✓/●/○）                                                          | `Agent/TaskTimelineView`。AI 操作が実際に step を進める（飾りではない）。`GenieState.activeTask` だけを描く                                                                                                 | `shots` 10 面目                                         |
...
| 9         | Chrome Extension → Native Messaging → Genie。**DOM 全文を送らない**                               | `apps/browser-extension`（見えているブロック 12 件 / 各 400 字 / 選択 2000 字まで）+ `Context/NativeMessagingHost`（4byte LE + JSON、1MB 上限）。受け側でも上限で切る                                       | `browser`                                               |
...
| 1         | Developer ID 署名 + hardened runtime + DMG                                                        | `scripts/build-dmg.sh`。署名後に **hardened runtime / Developer ID / entitlement を検査**してから DMG を作る。実測: `Genie-0.1.0.dmg` 1.4MB、`flags=0x10000(runtime)`、`Authority=Developer ID Application` | 手動（`bash scripts/build-dmg.sh`）                     |
...
| 仕様書                                     | Genie の実体                                                                            |
| ------------------------------------------ | --------------------------------------------------------------------------------------- |
| `App/`                                     | `App/`（`GenieAppDelegate` / `SelfTest` / `DemoMode`）                                  |
| `Core/State`, `Core/Events`, `Core/Models` | `Core/`（`GenieStateStore` / `GenieEventBus` / `GenieModels`）                          |
