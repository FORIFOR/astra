...
実測して分かった不足（§17.1 の表に無いが、無いと §19 の AA を満たせない）:

| 追加 token                  | 理由                                                                                                                                                             |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `accentOn`                  | dark の accent `#8A7DFF` 上の白は **3.25:1** しかなく通常テキストの AA(4.5) に届かない。primary button は accent 背景なので、暗い前景（`#0F1115`、5.81:1）へ倒す |
| `focusRing`                 | §19「focus ring を消さない」。見えない ring は消したのと同じなので、全 surface で 3:1 以上を検査する                                                             |
| `surfaceRaised` / `overlay` | popover と modal の背面。§17.3 の Glassmorphism 限定方針を守るために面を分ける                                                                                   |

仕様の値をそのまま使えたもの: light / dark の 9 色すべてが canvas・surface 上で AA を満たす
（最小は light の muted で 4.68:1）。タイポ・spacing・radius・motion の各値も範囲内。

`space` は 8px グリッドを厳密に保ち、`padding.cardLoose` の 20px（§17.3 が明示）だけを
例外として padding 側に置いた。8 の倍数でない値を spacing に足さない。

レイアウト判定（§7.2）は `resolveLayout(width, userCollapsed)` に閉じ込め、
...
`packages/contracts` の client 側。**応答は必ずスキーマで検証してから返す。**
サーバを信用して素通しすると、契約のずれが UI の奥深くで初めて露見する。

- エラー本文が契約どおりでなければ、**中身を推測しない**。
  proxy の HTML や 502 ページを `code` として扱うと、分岐が嘘の code で動き出す。
- `Idempotency-Key` は呼び出し側が渡せる。再送で同じ鍵を使えることが冪等性の意味で、
  毎回作り直しては意味がない。
- 401 の回復は**一度だけ**。無限に繰り返すと失効した資格情報で叩き続ける。

#### SSE クライアント（§7.3 のクライアント側）

`EventSource` を使わない。独自ヘッダ（Authorization）を付けられないため、
`fetch` + ReadableStream で読む。

| 性質                                       | 理由                                                                       |
| ------------------------------------------ | -------------------------------------------------------------------------- |
| 欠番を見つけたらその番号から取り直す       | 連続性はサーバとの契約。クライアントが握り潰すと取りこぼしに誰も気づけない |
| 重複は捨てる                               | 再接続の境界で同じイベントが二度届き得る                                   |
| 一度でも進んだら再接続の試行回数をやり直す | 長時間タスクで序盤の失敗が後半の予算を食わないように                       |
| 試行上限で諦める                           | 落ちているサーバを永久に叩かない                                           |
| フレームは chunk 境界をまたいで再結合      | 1 イベントが 2 つの chunk に割れるのは普通に起きる                         |

#### Work Surface（§6）

イベント列 →「仕事の進行」への畳み込みを純粋関数 `applyEvent` に閉じ込めた。

- 表示するのは **仕事の工程**。Agent 名・tool 名・worker 数を view に持ち込まない（§6.1）。
- **進捗率は真の進行率が計算できるときだけ。**段数が決まらない処理では `null` にして
  推定 % を出さない。代わりに step state + detail（「12 sources」）+ 経過時間で表す（§6.2）。
- **承認待ちは進捗と混ぜない。**別の attention state として持つ（§6.2）。
- retry 中の step は失敗として固定せず「再試行中」に置き換える（§6.2）。
- 主ボタンは「承認」ではなく結果を書く（§14.1）。サーバが `primary_action_label` を持つ（D-18）。
- 失敗表示は「AI が失敗しました」で終わらせない。仕事への影響と次の選択肢を書く（§21）。
- **停止は dismiss と別の明示操作**（§4.4）。進行中のときだけ出す。

### 19.8 UI-3 で確定したこと

#### トークンの持ち方（実装仕様 §4.2 のクライアント側）

| 種別          | 置き場所                                                              |
| ------------- | --------------------------------------------------------------------- |
| access token  | **メモリのみ**。寿命 15 分                                            |
| refresh token | Tauri は OS の資格情報ストア（`keyring`）。**ブラウザでは保存しない** |

ブラウザに安全な保管先が無い以上、`localStorage` へ置いて「保存できている」ことに
するより、保存しない方が正しい（再読み込みでサインアウトになるが、
refresh token をディスクへ平文で残すより望ましい）。テストで実際に
`localStorage` へ漏れていないことを検査している。

401 を受けたら**一度だけ**ローテーションして同じ要求をやり直す。
失効していたら（再利用検知など）保存済みの値を消してサインアウトさせる。

#### CORS（逸脱 D-21）

Vite（1420）も Tauri の webview（`tauri://localhost`）も api-gateway とは別オリジンなので、
CORS を設定しないと UI からは一切叩けない。

- **許可オリジンの既定は空。**`*` を既定にすると、認証済みの API が任意のサイトから叩ける。
  development / test でのみ Vite と Tauri の webview を自動で通す。
- `Access-Control-Expose-Headers` に `x-request-id` とレート制限のヘッダを載せる。
  露出させないとクライアントが読めない。
- 許可メソッドを**明示する**。既定のままだと `DELETE` が preflight に載らず、
  ブラウザからの uninstall が弾かれる（実際に踏んだ）。

#### Home の静けさ（正本 §2.1 / UI/UX §8.1）

`ProactiveScore = Importance × Urgency × Confidence × UserRelevance - InterruptionCost`
を実装し、**出さない判断も含めて 1 箇所**（`buildAttentionFeed`）でやる。
画面側に「これは出すべきか」を散らすと、静けさの基準がぶれる。

出さないもの: 進行中の仕事（常に何か出続けて意味がなくなる）/ すぐ終わった仕事
（§8.1 が挙げるのは「完了した長時間 Task」）/ 24 時間より前の出来事。
最大 3 件で、4 件目以降は「すべて見る」へ送る。

#### task → artifact の連続性（UI-3 の Exit）

タブ移動と「何を見せるか」を一体で扱う（`openTask` / `openArtifact`）。
移動だけして何も選ばれていない画面に落とすと、利用者が探し直すことになる。
Work から成果物へ、Library の来歴から元の仕事へ、双方向に辿れる。
指定された仕事が現在の絞り込みに入っていなければ、絞り込みを `すべて` へ切り替えて必ず見せる。
