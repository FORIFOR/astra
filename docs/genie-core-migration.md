# Genie Core 移行（Phase 1 完了）

## 何をしたか

OS/UI 非依存のロジックを共通 Rust crate **`genie-core`** に切り出し、macOS Swift アプリから
...
shared/design/tokens.json ─┐
                    ...
                    genie-core (Rust, OS/UI 非依存)
                           │  UniFFI
        ┌──────────────────┼───────────────────┐
   macOS SwiftUI      apps/desktop (Tauri)   Windows WinUI(未ビルド)
   GenieCoreBridge     meeting.rs が core を     GeneratedMetrics.cs
   → GenieCore         plain Rust として利用      + 設計メモのみ
```
...
- `GenieMode`（Idle/Listening/Thinking/Recording/RecordingPaused/Processing）— 両 OS 共通の状態語彙
...
- `pnpm gen:swift-bindings`（`--check` で CI 鮮度検査）: `cargo build` → `uniffi-bindgen` →
  `apps/genie-macos/Sources/GenieCore/genie_core.swift` と `Sources/GenieCoreFFI/include/*`
...
to_wire/LinkState/定数）を削除し `use genie_core::…` に置換。frontend への wire 表現（LinkState snake_case、
...
- **共通**: genie-core の Record/Enum/関数（`recording_snapshot` / `to_wire` / `format_elapsed` /
  `scan_recoverable` / `GenieMode`）。C# へは UniFFI の C# backend か C ABI で同じ core を呼ぶ。
...
1. Journal の実書き込みも core 経由に寄せる（今は src-tauri が core の `Journal` を使用、macOS Swift 側の
   録音実書き込みは未接続）。
2. genie-core の C# binding 生成（`uniffi-bindgen` C# もしくは C ABI）を Windows 実機で検証。
3. macOS Swift の録音実データ接続（AVAudioEngine → to_wire → gateway）を core 経由に。
4. RAG / agent planning の pure model を次の slice として core へ。
5. SwiftPM の Rust ライブラリ参照は今 `-L ../../core/genie-core/target/debug` の相対 unsafeFlags。
...
- 検証: `pnpm verify:macos-recording`（`GenieMac --selftest record`）で Swift→core→ディスクを headless E2E。
...
- **C ABI**: `core/genie-core/src/capi.rs` + 手書きヘッダ `core/genie-core/include/genie_core.h`。
...
- **WinUI 3 solution 完成（build 未検証）**: `apps/windows/Genie.sln` + `Genie/Genie.csproj`（WindowsAppSDK）。
  ...
  - `CoreBridge/GenieCore.cs`: C ABI を P/Invoke（`RecordingSession` は macOS RecordingRuntime と同じ core を叩く）。
  - 寸法は `GeneratedMetrics.cs`（tokens 由来）。
- **CI**: `.github/workflows/windows.yml` が windows-latest で cargo build → dll 配置 → `dotnet build Genie.sln`。
...
- **.app パッケージング**: `pnpm build:macos-app`（`scripts/build-macos-app.sh`）で
  `Genie.app`（`com.astra.mac`、`LSUIElement`、NSMicrophone/AppleEvents/Calendars UsageDescription、ad-hoc 署名）を生成。
  `open Genie.app` で起動確認済み。**これで live 許可のダイアログを出せる形になる。**
...
- `genie-core::transcript` を新設: `LiveWindow`（窓/ホップのセグメント化と検証）・`TranscriptEvent`
...
- Tauri の `stt/recognizer.rs` はローカル定義を削除し `pub use genie_core::{LiveWindow, TranscriptEvent, merge_overlap}`
...
- 配線: `WindowCoordinator` に `isRecording` と `toggleRecording()` を追加、`GenieAppDelegate` が起動時に登録。
- 検証: `GenieMac --selftest shortcut` を追加し `scripts/verify-macos-recording.sh` に組み込み。
...
- `Audio/SystemAudioCapture.swift` を新設。**ScreenCaptureKit** の `SCStream` 音声出力を
  ...
  `excludesCurrentProcessAudio=true` で Genie 自身の音は除外。CMSampleBuffer → AVAudioConverter で 16k mono 化。
...
- `genie-core::oauth` を新設: `CallbackParams`（折り返しで戻る値の型）・`parse_callback`（クエリ解析）・
...
- Tauri の `oauth.rs` はローカル定義（型・parse・decode・URL 判定）を削除し `pub use genie_core::{…}` と
  `genie_core::is_allowed_auth_url(&url)` へ差し替え。**待ち受け（TcpListener）とブラウザ起動（`open`）は OS 統合として残す**
...
- macOS の RAG ドロワーは静的チップだけで core を呼んでいなかった。`GenieCoreBridge.rankContext` を追加し、
...
- `genie-core::connector` を新設（§1「connector contracts」）。**live なトークン交換は持たない**（提供者ごとの
  ...
  - TypeScript `@genie/oauth`（flow/pkce/providers）を Rust 契約へ写したもの。`client_secret` は持たない（RFC 8252 §8.5）。
...
- `capi.rs` に `genie_core_pkce_challenge` / `genie_core_authorize_url`（文字列 in/out、非 loopback・空 client_id・
  未知 provider は NULL）を追加し、`include/genie_core.h` に宣言。**Windows(C#/P-Invoke) が Swift と同じ core の
  connector 契約層を使える**ようにした（uniffi は C# 非対応のため安定 C ABI 経由）。
- `apps/windows/Genie/CoreBridge/GenieCore.cs` に P/Invoke 宣言と `GenieCore.PkceChallenge` / `GenieCore.AuthorizeUrl`
...
- `genie-core::connector` に `TokenSet` / `token_exchange_body`(RFC 6749 §4.1.3, **code_verifier を省かない**) /
  `parse_token_response`(error を握り潰さない・**期限/scope を推測しない**) / `exchange_code`(token endpoint へ POST) を追加。
  TS `@genie/oauth` の flow.ts を Rust 契約へ写した。
...
- `capi.rs` に gateway API の C ABI を追加: `genie_core_api_reachable` / `_dev_sign_in`(Tokens JSON) / `_me`(Me JSON) /
...
- `include/genie_core.h` に宣言、`GenieCore.cs` に P/Invoke + `ApiReachable/ApiDevSignIn/ApiMe/ApiCreateMeeting/
...
- **空実装の修正**: `RecordingWorkspaceState.captureScreenshot()` は `{}`（空）だったが、TaskDock の viewfinder
  ...
  会議フォルダ `Genie/meetings/<id>/screens/screen-*.png` に PNG 保存（ImageIO）。
...
- **未実装機能の追加**: `scan_recoverable`（core）はテストのみで、アプリは回復候補を**一切 surface していなかった**。
  ...
  - 起動時に `GenieAppDelegate` がスキャンし、あれば `RecoveryState.shared.pending` に積む（NSLog で通知）。
...
- `connector_parse_callback` は uniffi(Swift)にだけ公開していたので、**C ABI に `genie_core_parse_callback`
  （折り返し URL → JSON）を追加**し、Windows C# も同じ解析を使えるようにした。header・`GenieCore.cs`
  （`ParseCallback`）・`verify-c-abi.sh` を更新。
- 検証: **C(clang) から** `genie_core_parse_callback("/callback?code=abc&state=xyz")` → JSON に code/state を確認。
...
- 修正: core に `mark_meeting_uploaded(root, meeting_id)`（journal を Uploaded に）を追加（uniffi 公開）。
  Swift の `end()`/`recover()` がアップロード成功後に `GenieCoreBridge.markUploaded` を呼ぶ。
...
- Windows の回復フローも二重アップロードを防げるよう、C ABI に `genie_core_mark_meeting_uploaded(root, meeting_id)`
  を追加（header・`GenieCore.cs` の `MarkMeetingUploaded`）。
...
- **Done#8 の正確な確認**: 最終製品＝**macOS native app は core を直接使い Tauri に一切依存していない**
  （`apps/genie-macos` に `import Tauri`/`WebviewWindow`/`AppHandle`/`apps/desktop`/`src-tauri` の実コード参照ゼロ、
  `genie-core` も tauri crate 非依存）。これまで「live トークン交換が残る」と書いたが、それは **Tauri 依存ではなく
  外部 OAuth 提供者依存**であり、native の交換経路は core の `exchange_code` を通る（Tauri を介さない）。
- `scripts/check-native-tauri-free.mjs` を新設: native app の Swift（コメント除去後の実コード）と genie-core の Cargo を
  走査し、Tauri 参照があれば fail。`ci.yml` に `pnpm check:native-tauri-free` を追加。
- 検証: **実測 PASS** `native product path is Tauri-free: 45 Swift files + genie-core に Tauri 依存なし`。
...
- `apps/windows/bridge-check`（Program.cs + csproj）と `scripts/verify-csharp-bridge.sh` を新設。
  Windows アプリと**同一の `GenieCore.cs`** を取り込み、**P/Invoke で実 `libgenie_core` に繋いで**
...
- `apps/windows/Genie/AppLogic/GenieSession.cs` を新設（WinUI 非依存の純 C#、macOS の `MainData` 相当）:
  `SignIn`（tokens 取り出し、refresh は Keychain 相当へ）/ `Apps` / `Library` / `RunEchoTask`（Agent 往復）を
  C# ブリッジ経由で実 gateway に繋ぐ。UI(Window)からはこれを使う設計。
- `verify-csharp-bridge` を拡張し、gateway 到達時に **GenieSession を C# から往復検証**。
  **実測 PASS**: `CS_OK gateway(GenieSession): signedIn apps=12 library=0 echoArtifact=52bytes`
...
- `apps/windows/Genie/AppLogic/WindowsGlobalShortcut.cs` を新設。macOS の `GlobalShortcut`（Carbon）に対応する
...
- `apps/windows/Genie/AppLogic/WasapiCapture.cs` を新設。macOS の MicCapture(AVAudioEngine)/SystemAudioCapture
...
## 追記: Windows Main Window を GenieSession に配線（Phase 1.61, §4 Main Window）

- `MainWindow` が起動時に `GenieSession`（core→gateway、runtime 検証済み）でサインインし Apps/Library を取得、
  NavigationView の選択でセクション切替（macOS の MainData/MainWindowView と対）。Page への描画は WinUI 側（Windows）。
- WinUI Window コードのためビルドは Windows のみだが、**データ層 `GenieSession` は実 gateway で runtime 検証済み**。
...
- `apps/windows/Genie/AppLogic/WindowsCredentialStore.cs` を新設。macOS の KeychainStore/SessionStore に対応する
...
- `GenieSession.SignIn` が Windows のとき refresh/device token を Credential Manager に保存（`OperatingSystem.IsWindows()`
...
- `apps/windows/Genie/AppLogic/WindowsScreenCapture.cs` を新設。macOS の ScreenContextCapture(CGDisplayCreateImage)
...
- `dotnet build apps/windows/Genie.sln` を試すと `NETSDK1100`（Windows 対象は `EnableWindowsTargeting=true` が要る）。
...
- `GeniePanel` は `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]`・borderless・透過・
...
- `scripts/verify-all.sh`（`pnpm verify:all`）を新設。**この環境で検証できる全ゲートを 1 コマンドで**通す:
  genie-core 35 / Tauri Rust 67 / Tauri desktop JS 352 / design-tokens・swift-bindings・workspace-fixture の鮮度 /
...
- 検証: **実測 PASS** `CSLOGIC_OK: Windows C# 実ロジック全体(Window code-behind 含む)が型検査を通過`。
  対象: GenieCore(bridge)／GenieSession／WASAPI／GlobalShortcut／CredentialStore／ScreenCapture／geometry ＋
...
- 「Speech 認可は bundle id 紐づけなので、裸実行でなく署名済み `.app` から実行すれば STT が出るのでは」という仮説を検証。
  署名済み `Genie.app/Contents/MacOS/Genie --selftest sttrecognize`（NSSpeechRecognitionUsageDescription 入り Info.plist・
...
- `apps/windows/Genie/Genie.csproj` に **`<WindowsPackageType>None</WindowsPackageType>`**（＋`EnableMsixTooling`）を追加。
  WinUI 3 は既定で MSIX packaging を要し、CI の素の `dotnet build apps/windows/Genie.sln -c Release -p:Platform=x64` が
...
- `apps/windows/Genie/app.manifest` を追加し `PerMonitorV2`（+ `dpiAware=true/pm` 後方互換、
  `longPathAware`、Windows 10/11 `supportedOS`）を宣言。
- `Genie.csproj` に `<ApplicationManifest>app.manifest</ApplicationManifest>` を配線。
...
- `apps/genie-macos/Sources/GenieMac/Windowing/GlobalShortcut.swift` — `CGEvent.tapCreate`
...
| 3   | macOS 実機 E2E PASS                           | ✅（大半）/ 🟡（カレンダー）              | 実マイク録音·実 screen·会議E2E·回復·波形·**STT テキスト(file+streaming)**·**global shortcut 合成押下受信**·Keychain·AX·on-device STT。**カレンダー実データも実測 PASS**（署名 .app + ユーザー TCC 許可 → 実イベント 6 件取得, 2026-08-28）。                       |
...
| 5   | Windows build 可能 solution/CI 完成           | ✅（構成）/ 🟡（実行）                    | Genie.sln(Debug/Release x64)·unpackaged csproj·PerMonitorV2 manifest·windows.yml(cargo→dll→dotnet build)。**残**: windows-latest / 実機での実行。                                                                                                                  |
...
- **Voice HUD**（GeniePanel, VoiceHUDView）: token 実寸 **310×31**、実測 **108 色**。
- **Recording Workspace**（GeniePanel, RecordingWorkspaceView）: token 実寸 **920×590**、実測 **83 色**。
...
private→(ユーザー承認で)public リポジトリ `FORIFOR/genie` を作成し、GitHub Actions で
...
2. **cdylib 名**: Rust の cdylib は Windows で `genie_core.dll`（lib 接頭辞なし）。bridge の
   ライブラリコピーが `libgenie_core.dll` しか探さず DllNotFound。両名を探すよう修正。
3. **bridge 判定**: gateway 不在の CI で先頭行が `CS_SKIP` になり、先頭一致の成功判定が誤失敗。
   「core bridge の CS_OK があり CS_FAIL 無し」を条件に変更。
4. **ImplicitUsings**: 実 `Genie.csproj` が未設定で `IntPtr/Guid/...` が CS0246。logic/bridge-check
...
## Phase 1.83 — Windows 実行時スモーク: Genie.exe 起動 + Voice HUD 実描画（Done#4 実行時）
...
- `dotnet publish --self-contained -p:WindowsAppSDKSelfContained=true`（WindowsAppSDK runtime 同梱、
  unpackaged でも起動可）→ 実 `Genie.exe` を windows-latest で起動 → 12 秒生存を確認 → プライマリ画面を撮影。
- 結果: **`SMOKE_OK: Genie.exe stayed alive 12s (WinUI bootstrap + window created)`**。
...
`windows` workflow に実行時スモーク（self-contained publish → Genie.exe 起動 → 撮影）を入れ、
