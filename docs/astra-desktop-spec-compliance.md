...
| 1         | Developer ID 署名 + hardened runtime + DMG                                                        | `scripts/build-dmg.sh`。署名後に **hardened runtime / Developer ID / entitlement を検査**してから DMG を作る。実測: `Astra-0.1.0.dmg` 1.4MB、`flags=0x10000(runtime)`、`Authority=Developer ID Application` | 手動（`bash scripts/build-dmg.sh`）                     |
