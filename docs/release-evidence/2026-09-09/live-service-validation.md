...
- The 1055f32 notarized artifact passed 51/51 distribution checks and invocation 9/9; first microphone audio was 195ms. Subsequent validation-harness corrections require final source provenance to be rebuilt.
- Microsoft seed creation now requests a received-message fixture using the initial MAPI message flags and requires Graph to return `isDraft: false`. A draft or absent state stops validation while retaining cleanup IDs. This is a synthetic fixture import, not a setup send or proof of real receipt; live provider confirmation remains pending. Agent Host tests: 143 passed; TypeScript typecheck passed.

- The notarized 0.1.2 native candidate at e2febeb passed all 51 distributed-artifact checks with zero skips, and full verification with the distribution explicitly selected passed. First microphone audio was 169.9ms; invocation 9/9 and audio state truth passed. The dfcdf96 rebuild also passed all 51 distributed-artifact checks. Final artifact provenance must follow the remaining backend/CI corrections.
- After schema generation passed on GitHub, CI exposed host-timezone dependence in deadline extraction, daily meeting labels and weekly workload. The Japanese business calendar is now explicitly Asia/Tokyo, with named-zone support in deadline parsing; explicit ISO offsets are preserved, and invalid/ambiguous wall times are rejected. UTC and America/Los_Angeles regressions pass without setting the production process timezone. Typecheck and the final UTC database-backed workspace suite pass: 2,201 passed, one skipped. GitHub CI passed those tests and acceptance, then exposed a stale smoke-test catalog count (12 instead of the 14 bundled plugins). The corrected real-bootstrap smoke passes locally; CI confirmation is pending.
