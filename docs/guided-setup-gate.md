...
macOS の権限設定を、右下の Genie アバターが System Settings の対象までハイライトと吹き出しで案内する。
...
| `GenieMac --selftest guidedsetup --simulate-not-granted` | 実 System Settings、実 AX、権限は変えない。対象の特定、CGEvent の実ドラッグへの追従、窓 0 / 監視 0                                                              |
| `GenieMac --selftest guidedshots`（Atlas）               | 8 面を RC に描かせて撮る。GUIDED_SETUP_GEOMETRY: highlight ⊇ target、bubble ∩ toggle = ∅、bubble ∩ − = ∅、bubble ⊂ visibleFrame、未発見時に highlight/callout 0 |
| `scripts/ui-atlas/review-blind.sh`（ASTRA_JUDGE_ONLY）   | Genie が描いた面だけを 3 model が盲検（合成 3 面は `blind_review: false`、文脈証拠）                                                                            |
...
5. 吹き出しが行の名前と別名（Genie / GenieDbg）で呼んだ → 見つけた行の名前で言う。
6. 大きな丸に「!」を描くと飾りに見えた → 丸は Genie の印だけ、状態は輪の色と吹き出しの記号。
7. 吹き出しに尾が無く指し先が繋がらない → `CalloutBubbleShape` の尾が対象（またはアバター）を指す。

## 環境に由来し、Genie では直せないもの

- この Mac の TCC 一覧には dev バンドル「GenieDbg」と第三者アプリ（「2.1.260」等）が並ぶ。judge はそれを欠陥と読む。
  合成 3 面を盲検から外し、幾何 gate と切り出し面（target-highlighted）で Genie の画素だけを見る。専用のテストアカウント
...
| 判定                                            | 結果                                       |
| ----------------------------------------------- | ------------------------------------------ |
| VISUAL_IDEAL_GATE（blind、Genie が描いた 5 面） | PASS — KEEP 5 / FIX_CANDIDATE 0 / NEE 0    |
