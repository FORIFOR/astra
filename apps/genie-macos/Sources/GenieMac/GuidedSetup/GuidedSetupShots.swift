...
///
///   guided-setup-intro.png               右下のアバター + 導入の吹き出し（Genie の窓だけ）
///   guided-setup-target-found.png        System Settings の窓 + ハイライト + 吹き出し（合成: Apple の窓と Genie の 2 窓）
...
/// dark 指定は Genie の窓にだけ効く（System Settings は OS の外観のまま）ので、合成の 3 面は light だけ撮る。
...
extension SelfTest {
    ...
    static func guidedShots(_ args: [String]) {
        ...
        /// Apple の窓（System Settings）と Genie の窓を、**それだけ**で合成する（机の上の他の窓や壁紙は入れない）。
        ...
        func composite(settingsPID: pid_t, overlays: [NSWindow]) -> CGImage? {
            ...
            for w in overlays {
                // Genie の窓の位置は自分が知っている（AppKit → CG 上原点）。window list に載る前でも撮れる。
                let f = w.frame
        ...
        if AXElementService.shared.isTrusted {
            ...
            var depsAdd = deps
            depsAdd.appNames = ["Genie"]   // 案内の文言に出す名前。一覧の照合には次の行で当たらない名前を使う。
