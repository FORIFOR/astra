...
final class PermissionGuideCoordinator: ObservableObject {
    ...
    struct Dependencies {
        ...
        /// Hide only Genie's ordinary windows; restoring never resumes a task or records audio.
        ...
        static func bundleAppNames() -> [String] {
            ...
            names.append("Genie")
            // macOS may retain the old display name for an existing TCC grant.
            names.append(contentsOf: ["Astra", "AstraMac"])
            var seen = Set<String>()
    ...
    private func enableExplainedPermission() {
        ...
        subscribeSystemEvents()
        // Recheck immediately before asking: another Genie instance may have received the grant.
        if deps.permissions.state(of: permission) == .granted { showReady(permission); return }
    ...
    private func guideMicrophoneInSettings() {
        ...
        settingsWaitStarted = deps.now()
        locateAndShow(selectors: SystemSettingsAnchorLocator.genieRowSelectors(appNames: deps.appNames),
    ...
    /// Genie 行 → 無ければ「+」→ 無ければ一般ガイド。**1 回の取り直しで 1 回だけ描く**
    /// （行が無いときに「消して→+ を描く」を毎回すると、AX の出来事ごとに吹き出しが点滅した）。
    private func locateScreenCaptureTargets() {
        locateAndShow(candidates: [
            (SystemSettingsAnchorLocator.genieRowSelectors(appNames: deps.appNames), Self.calloutTurnOn),
    ...
    private func locateAndShow(candidates: [([AXSelector], String)], fallback: String?) -> Bool {
        ...
        if let found = result.anchor {
            ...
            // 吹き出しは見つけたものの名前で言う（一覧の行が「GenieDbg」なら「GenieDbg をオンにしてください」。別名で呼ぶと誤読させる）。
            let app = found.match.node.displayName ?? deps.appNames.first ?? "Genie"
    ...
    static let messageAccessibility = "アクセシビリティの許可を確認しています\nシステム設定の一覧で Genie をオンにしてください"
    static let messageScreenCapture = "画面収録が未許可です\nシステム設定で Genie をオンにしてください"
    static let messageMicrophone = "マイクが未許可です\n許可すると声で頼めます"
    static let messageMicrophoneSettings = "マイクが未許可です\nマイクの設定で Genie をオンにしてください"
    static let messageGeneralTurnOn = "Genie を自動で見つけられません\n画面収録の一覧で Genie をオンにしてください"
    static let messageMicrophoneFallback = "Genie を自動で見つけられません\nマイクの一覧で Genie をオンにしてください"
