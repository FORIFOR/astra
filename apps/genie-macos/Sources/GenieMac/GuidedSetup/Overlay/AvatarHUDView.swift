...
///
/// 文法: System Settings = 主役、対象行 / toggle = 操作対象、この callout = ガイド、Genie の署名 = カード内の小さな mark。
...
struct AvatarHUDView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 6) {
                GenieVoiceMark().scaleEffect(0.75).frame(width: 18, height: 12)
                Text("Genie")
            ...
            if let application = model.applicationToAdd {
                Button(model.showsApplicationHelp ? "追加の案内を閉じる" : "Genieが一覧にありませんか？") {
                ...
                if model.showsApplicationHelp {
                    VStack(alignment: .leading, spacing: Space.compact) {
                        ...
                        Button("FinderでGenieを表示") { application.reveal() }
                            .font(.system(size: S.type(TypeScale.captionSize)))
                            .accessibilityIdentifier("guideRevealApplication")
                        Text("許可後は自動で確認します。変わらない場合はGenieを開き直してください。")
                            .font(.system(size: S.type(TypeScale.captionSize)))
                            .foregroundStyle(Palette.muted(dark))
                            .fixedSize(horizontal: false, vertical: true)
                    }
