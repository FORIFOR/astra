...
extension Color {
    /// Genie のアクセント。§17.1 の生成トークン（tokens.json → Palette）を単一正として使う。
    /// 直書きの色は持たない（spec: UI を各 OS へ直書きせず tokens から生成する）。
    static let genieAccent = Palette.accentLight
    /// テーマ対応のアクセント（Light/Dark で切替）。
    static func genieAccent(_ dark: Bool) -> Color { Palette.accent(dark) }
