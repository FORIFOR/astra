...
extension SelfTest {
    ...
    static func screenshotE2E(_ args: [String]) {
        ...
        try? FileManager.default.createDirectory(atPath: outDir, withIntermediateDirectories: true)
        guard GenieCoreBridge.reachable(base) else { print("SCREENSHOT_E2E=SKIP gateway unreachable at \(base)"); exit(0) }
        ...
        do {
            let tokens = try GenieCoreBridge.devSignIn(base, email: email, displayName: "E2E")
            ...
            let t0 = Date()
            let outcome = try GenieCoreBridge.sendTurn(base, accessToken: tokens.accessToken, conversationId: conv, text: question, attachments: attachments)
...
extension SelfTest {
    ...
    static func screenshotShots(_ args: [String]) {
        ...
        func shoot(_ name: String) {
            ...
            let want = GenieStateStore.shared.dock.size()
            report.append("\(name) \(Int(win.frame.width))x\(Int(win.frame.height)) want=\(Int(want.width))x\(Int(want.height)) dock=\(GenieStateStore.shared.dock) headless=\(WindowCoordinator.headless)")
        }
