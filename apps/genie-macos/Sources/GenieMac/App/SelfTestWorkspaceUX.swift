    ...
    @MainActor static func workspaceUX(_ args: [String]) async {
        ...
        for dark in [false, true] {
            for width in [940.0, 1162.0] {
                ...
                var record = TaskRequestRecord(request: "30秒のGenie紹介動画の構成を3案。撮影は画面収録のみ。", base: "fixture")
                record.phase = .working; record.message = "結果を待っています。別の画面で作業を続けられます。"
                var outcome = AgentTask(requestRecord: record, id: UUID(), title: "30秒で、Genieの価値を伝える", status: .running, steps: [], startedAt: task.startedAt, context: ContextBundle())
