...
extension SelfTest {
    ...
    @MainActor static func outcomeLive(_ args: [String]) async {
        ...
        do {
            ...
            let prompt = "Genieを紹介する30秒の縦型動画の構成案を日本語で3案作ってください。Genieで使える機能はHomeで依頼文を入力、Workで完成した文章を開く、コピー、Markdown保存の4つです。コードエディタや動画の編集機能はありません。対象は個人開発者。目的は、依頼から成果物まで1つのアプリで進められる価値を伝えること。撮影素材はGenieの画面収録のみで、予算0円。各案にタイトル、冒頭3秒の見せ方、3〜22秒の展開、22〜30秒の締め、撮影に必要な操作を記載してください。最も伝わる1案と理由も選んでください。再生数や売上の保証はしないこと。動画ファイルの生成やSNS投稿はせず、構成案の文章だけを作成してください。"
            let id: UUID
            ...
            for _ in 0..<180 {
                ...
                if record.hasResult {
                    ...
                    let documentURL = URL(fileURLWithPath: root).appendingPathComponent("Genie-video-concepts.md")
                    ...
                    if args.contains("--interactive") {
                        ...
                        window.isReleasedWhenClosed = false; window.title = "Genie · 実際に作成した構成案"
