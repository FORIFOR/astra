...
struct RecordingNowCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 10) {
                ...
                Text(Facts.recordingHeroRecording)
            ...
            HStack(spacing: 8) {
                ...
                Button("Ask Genie") { WindowCoordinator.shared.openMeetingPanelFromHome(.ask) }
                    .font(.system(size: S.type(TypeScale.secondarySize), weight: .medium))
                    .foregroundStyle(Palette.text(dark))
                    .frame(height: 32).padding(.horizontal, 14)
                    .buttonStyle(GenieControlStyle(radius: 8, base: 0.05))
                ...
                Spacer(minLength: 0)
                StopButton(label: Facts.recordingStop,
    ...
    private var subtitle: String {
        ...
        if session.participantCount > 0 { parts.append("\(session.participantCount) 人") }
