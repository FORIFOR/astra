            ...
            VStack(alignment: .leading, spacing: Space.largePadding) {
                ...
                HStack(spacing: 6) {
                    ForEach(MeetingArtifactView.tabs, id: \.self) { t in
                        ...
                        .buttonStyle(GenieControlStyle(radius: 8, base: tab == t ? 0.10 : 0.04))
    ...
    private func citationList(_ items: [MeetingCitation]) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ForEach(items) { c in
                ...
                .buttonStyle(GenieControlStyle(radius: 7, base: isShown(c) ? 0.08 : 0.0))
            }
        }
    }
            ...
            ForEach(items) { c in
                HStack(alignment: .top, spacing: 6) {
                    ...
                    if let n = c.number {
                        ...
                        .buttonStyle(GenieControlStyle(radius: 6, base: isShown(c) ? 0.10 : 0.0))
                        .help("出所の発言を見る")
                    }
                    Spacer()
                }
