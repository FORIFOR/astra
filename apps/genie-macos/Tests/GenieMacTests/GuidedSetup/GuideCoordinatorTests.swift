...
final class GuideCoordinatorTests: XCTestCase {
    ...
    func testHighlightAndCalloutNeverTakeMouseEventsAndStayOnScreen() {
        ...
        callout.show(message: "Genie をオンにしてください", near: target, screen: screen)
    ...
    func testRealOverlayStackReleasesEveryPanel() {
        ...
        let anchor = GuideAnchor(rect: CGRect(x: 200, y: 200, width: 40, height: 20),
                                 match: AXMatch(node: AXElementSnapshot(role: "AXCheckBox", title: "Genie"), selectorRank: 0))
        stack.showGuide(message: "Genie をオンにしてください", at: anchor, preferred: .left)
    ...
    func testMissingTargetShowsOnlyTheGeneralGuideAndDoesNotCrash() {
        let h = Harness(states: [.accessibility: .granted, .screenCapture: .notDetermined, .microphone: .granted],
                        tree: settingsTree(withGenie: false, withAdd: false))
        h.coordinator.start()
        h.coordinator.tick()
    ...
    func testAddButtonIsGuidedWhenGenieRowIsMissing() {
        let h = Harness(states: [.accessibility: .granted, .screenCapture: .notDetermined, .microphone: .granted],
                        tree: settingsTree(withGenie: false, withAdd: true))
        h.coordinator.start(); h.coordinator.tick()
        XCTAssertTrue(h.overlay.guideVisible)
        XCTAssertEqual(h.overlay.guideMessage, PermissionGuideCoordinator.calloutAdd(for: "Genie"))
    ...
    func testGuideFollowsWhenSystemSettingsMoves() {
        ...
        h.tree.tree = settingsTree(genieFrame: CGRect(x: 700, y: 400, width: 40, height: 22))
...
final class GuideAlreadyOnTests: XCTestCase {
    func testASwitchThatIsAlreadyOnIsNotAskedToBeTurnedOn() {
        ...
        let h = Harness(states: [.accessibility: .granted, .screenCapture: .notDetermined, .microphone: .granted],
                        tree: settingsTree(genieOn: true))
        h.coordinator.start(); h.coordinator.tick()
        XCTAssertEqual(h.overlay.guideMessage, PermissionGuideCoordinator.calloutAlreadyOn(for: "Genie"))
