...
final class AXElementServiceTests: XCTestCase {
    ...
    func testSelectorMatchesOnCombinedConditionsNotJustOneString() {
        let node = AXElementSnapshot(role: "AXCheckBox", title: "Genie", axDescription: "Genie のスイッチ", identifier: "row.genie")
        XCTAssertTrue(AXSelector(role: "AXCheckBox", titleAny: ["Genie"]).matches(node))
        XCTAssertFalse(AXSelector(role: "AXButton", titleAny: ["Genie"]).matches(node), "role が違えば title が合っても当てない")
        XCTAssertTrue(AXSelector(titleAny: ["gen"], titleContains: true).matches(node))
        XCTAssertFalse(AXSelector(titleAny: ["gen"]).matches(node), "完全一致では部分文字列に当てない")
        XCTAssertTrue(AXSelector(descriptionAny: ["switch", "スイッチ"]).matches(node), "日本語と英語の候補を並べられる")
        XCTAssertTrue(AXSelector(identifier: "row.genie").matches(node))
    ...
    func testSelectorsAreTriedInOrderAndTheFirstThatMatchesWins() {
        ...
        let match = tree.find(SystemSettingsAnchorLocator.genieRowSelectors())
        XCTAssertEqual(match?.node.identifier, "Genie_Toggle")
        XCTAssertEqual(match?.selectorRank, 0)
        // スイッチが無ければ名前の静的テキスト（後ろの selector）に落ちる。
        let noToggle = settingsTree(genieFrame: nil)
        var pruned = noToggle
        pruned.children[0].children[1].children.removeAll { $0.identifier == "Genie_Toggle" }
        let text = pruned.find(SystemSettingsAnchorLocator.genieRowSelectors())
        XCTAssertEqual(text?.node.identifier, "Genie_Title")
        ...
        XCTAssertEqual(tree.find(SystemSettingsAnchorLocator.genieRowSelectors(appNames: ["GenieDbg", "Genie"]))?.node.identifier, "Genie_Toggle")
        XCTAssertNil(tree.find(SystemSettingsAnchorLocator.genieRowSelectors(appNames: ["Other"])))
    }

    func testSelectorMatchesIdentifierListAndValue() {
        let toggle = AXElementSnapshot(role: "AXCheckBox", subrole: "AXSwitch", identifier: "Genie_Toggle", value: "0")
        XCTAssertTrue(AXSelector(role: "AXCheckBox", identifierAny: ["GenieDbg_Toggle", "Genie_Toggle"]).matches(toggle))
    ...
    func testLocatorReturnsNothingWhenNotTrustedAndDoesNotWalk() {
        ...
        let r = locator.locate(.accessibilityElement(bundleID: "x", selectors: SystemSettingsAnchorLocator.genieRowSelectors()))
    ...
    func testLocatorReportsWhyWhenTargetIsMissingOrHasNoFrame() {
        ...
        let tree = MockTree(); tree.tree = settingsTree(withGenie: false)
        let locator = SystemSettingsAnchorLocator(tree: tree, screenFrames: frames, primaryHeight: { 1080 })
        let missing = locator.locate(.accessibilityElement(bundleID: "x", selectors: SystemSettingsAnchorLocator.genieRowSelectors()))
        XCTAssertNil(missing.anchor)
        XCTAssertTrue(missing.reason?.hasPrefix("no match") == true)

        tree.tree = settingsTree(genieFrame: nil)
        let noFrame = locator.locate(.accessibilityElement(bundleID: "x", selectors: SystemSettingsAnchorLocator.genieRowSelectors()))
    ...
    func testLocatorConvertsToAppKitCoordinates() {
        let tree = MockTree(); tree.tree = settingsTree(genieFrame: CGRect(x: 600, y: 300, width: 40, height: 22))
        let locator = SystemSettingsAnchorLocator(tree: tree, screenFrames: { [CGRect(x: 0, y: 0, width: 1920, height: 1080)] }, primaryHeight: { 1080 })
        let r = locator.locate(.accessibilityElement(bundleID: "x", selectors: SystemSettingsAnchorLocator.genieRowSelectors()))
