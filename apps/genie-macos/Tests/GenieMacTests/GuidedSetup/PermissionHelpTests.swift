...
final class PermissionHelpTests: XCTestCase {
    ...
    func testMissingMicrophoneDoesNotSendUserToScreenRecording() {
        let h = Harness(states: [.microphone: .denied], tree: settingsTree(withGenie: false, withAdd: false))
    ...
    func testAnOnLookingSwitchCannotEstablishPermission() {
        let h = Harness(states: [.accessibility: .notDetermined], tree: settingsTree(genieOn: true))
        h.coordinator.guide(.accessibility)
    ...
    func testOnlyExistingAppDirectoriesCanBeOfferedAndPayloadKeepsExactPath() async throws {
        ...
        let app = root.appendingPathComponent("Genie 試験.app")
        try FileManager.default.createDirectory(at: app, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        XCTAssertNil(GuideApplication(url: URL(string: "https://example.com/Genie.app")!))
        ...
        XCTAssertEqual(application.name, "Genie 試験")
