...
/// 検査用の System Settings の木。Genie 行のスイッチが 1 つ。
func settingsTree(genieFrame: CGRect? = CGRect(x: 600, y: 300, width: 40, height: 22), withGenie: Bool = true, withAdd: Bool = true, genieOn: Bool = false) -> AXElementSnapshot {
    ...
    if withGenie {
        rows.append(AXElementSnapshot(role: "AXStaticText", identifier: "Genie_Title", value: "Genie", axFrame: CGRect(x: 420, y: 300, width: 90, height: 22)))
        rows.append(AXElementSnapshot(role: "AXCheckBox", subrole: "AXSwitch", identifier: "Genie_Toggle", value: genieOn ? "1" : "0", axFrame: genieFrame))
    }
...
struct Harness {
    ...
    init(states: [GuidePermission: Permissions.State], settingsRunning: Bool = true, tree: AXElementSnapshot? = settingsTree()) {
        ...
        deps.appNames = ["Genie"]          // 検査は xctest の中で走るので、バンドル名から取らない
