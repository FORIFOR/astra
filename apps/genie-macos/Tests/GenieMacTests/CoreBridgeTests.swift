import XCTest
import GenieCore
@testable import GenieCore

/// genie-core を Swift から使う契約テスト（bindings freshness と併せて回帰を止める）。
final class CoreBridgeTests: XCTestCase {
    func testVersionRoundTrip() {
        ...
        let cargo = URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent("core/genie-core/Cargo.toml")
        ...
        XCTAssertEqual(genieCoreVersion(), expected ?? genieCoreVersion())
        XCTAssertFalse(genieCoreVersion().isEmpty)
    }
