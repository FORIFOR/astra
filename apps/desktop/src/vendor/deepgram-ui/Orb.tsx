...
function rollingAvg(noise: number[], start: number): number {
  ...
  return win.length === 0 ? 0 : win.reduce((a, b) => a + b, 0) / win.length;
}
...
function drawFrame(
  ...
  lineConfigs.forEach((line, i) => {
    ...
    ctx.shadowColor = line.segments[0]?.color ?? 'transparent';
    ...
    const gradient = makeGradient(
      ...
      line.centerOffset,
      line.startAngle +
        ((shape.time * pi(1)) / 1000 / AVERAGE_ROTATION_PERIOD_SECONDS) * line.speedMultiplier,
      line.segments,
    );
    ...
    drawCrescent(
      ...
      deflation,
      pi(3 / 2) +
        Math.sin((shape.time * pi(2)) / ROCKING_PERIOD_SECONDS / 1000) * shape.rockingAngle,
      gradient,
    );
  });
}
...
function deflationFor(state: OrbState): number {
  switch (state) {
    case 'talking':
      return 0.55; // gentle crescent mouth
    case 'listening':
      return 0; // full circle
    case 'idle':
    default:
      return 1; // fully pinched
  }
}
...
function rockingFor(state: OrbState): number {
  switch (state) {
    case 'talking':
      return 0; // no rocking — mouth stays oriented
    case 'listening':
      return pi(1 / 15);
    case 'idle':
    default:
