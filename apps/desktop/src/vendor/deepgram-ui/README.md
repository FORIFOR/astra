...
AgentProvider / WebSocket / microphone（Deepgram API 前提。Genie は
Google STT / TTS と Local Agent Host を使う）。

書式は repo の Prettier に合わせて整形してある。ロジックは変えていない。
Genie 側の adapter は `src/voice/` にある。

## どこまで Deepgram の作法を取り込んだか（2026-08-27）

| Deepgram の要素                                                                   | Genie での扱い                                                                                   |
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `Orb` / `LiveWaveform`                                                            | そのまま（色だけ Genie の accent）。Dock の行と Voice HUD で描画                                 |
...
| dark scheme（#18181c / #222228 / #1e1e24 / #8b8b9a / border .08）                 | `@genie/ui-kit` の `floatingSurface` → `--genie-float-*`。Dock と HUD だけが使う                 |
...
| Deepgram の要素                                                                                 | Genie での扱い                                                                                                                                               |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Response`（依存なしの markdown 描画）                                                          | `vendor/deepgram-ui/Response.tsx`。解析はそのまま、装いは Genie の CSS。Dock の答えに使う                                                                    |
