...
/// データ（Apps/Library）は共通の GenieSession（core→gateway）から取る。macOS の MainData と対。
public sealed partial class MainWindow : Window
{
    private readonly GenieSession _session =
    ...
    private async Task LoadAsync()
    {
        await Task.Run(() =>
        {
            ...
            _session.SignIn($"win-{Environment.ProcessId}@genie.local", "Genie");
            Apps = _session.Apps();
