...
public sealed class GenieSession
{
    ...
    public GenieSession(string baseUrl) => _base = baseUrl;

    public bool Reachable() => GenieCore.ApiReachable(_base);
    ...
    {
        string json = GenieCore.ApiDevSignIn(_base, email, displayName);
    ...
    public string[] Apps()
    {
        string json = GenieCore.ApiPluginCatalog(_base, AccessToken);
    ...
    {
        string json = GenieCore.ApiLibrary(_base, AccessToken);
        return ParseStringArray(json);
    }
    ...
    {
        string taskId = GenieCore.ApiCreateTask(_base, AccessToken, "echo", $"{{\"message\":\"{message}\",\"steps\":1}}");
        if (string.IsNullOrEmpty(taskId)) return "";
        string statusJson = GenieCore.ApiWaitTask(_base, AccessToken, taskId, 15000);
        ...
        return GenieCore.ApiArtifactContent(_base, AccessToken, artifactId);
    }
