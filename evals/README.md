...
`blocking: true` のケースは CI を止める。現状 `actions/phase0` `phase2` `phase3` `phase4` `phase5` `phase6` `phase7` `security` `product` が blocking。
...
| `actions/product`                                   | 正本 §30 Product Acceptance の Case A（通し） | **blocking** |
