...
//!
//! UniFFI（Swift 用）と併存する。文字列は呼び出し側が `genie_core_string_free` で解放する。
//! セッションは不透明ハンドル（`genie_core_session_*`）。録音の実体は共通の `Journal`。
...
/// 版。疎通確認に使う。戻り値は genie_core_string_free で解放。
#[no_mangle]
pub extern "C" fn genie_core_version() -> *mut c_char {
    cstr(env!("CARGO_PKG_VERSION"))
}

/// 00:00 / 1:02:03。戻り値は解放が必要。
#[no_mangle]
pub extern "C" fn genie_core_format_elapsed(ms: u64) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_mark_meeting_uploaded(
    root: *const c_char,
...
///
/// # Safety
/// `genie_core_*` が返したポインタだけを一度だけ渡すこと。
#[no_mangle]
pub unsafe extern "C" fn genie_core_string_free(p: *mut c_char) {
    if !p.is_null() {
        drop(CString::from_raw(p));
    }
}

/// PKCE の code_challenge（RFC 7636 S256）。戻り値は genie_core_string_free で解放。
///
/// # Safety
/// `verifier` は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_pkce_challenge(verifier: *const c_char) -> *mut c_char {
    match rstr(verifier) {
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_authorize_url(
...
/// 例: {"code":"...","state":"...","error":null,...}。戻り値は genie_core_string_free で解放。
///
/// # Safety
/// `target` は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_parse_callback(target: *const c_char) -> *mut c_char {
...
// 文字列を返すものは JSON（複合型）または生値。失敗は NULL。すべて genie_core_string_free で解放。
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_reachable(base_url: *const c_char) -> i32 {
    match rstr(base_url) {
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_dev_sign_in(
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_me(
    base_url: *const c_char,
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_create_meeting(
    base_url: *const c_char,
    access_token: *const c_char,
    title: *const c_char,
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_create_task(
    base_url: *const c_char,
    access_token: *const c_char,
    kind: *const c_char,
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_wait_task(
    base_url: *const c_char,
    access_token: *const c_char,
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_artifact_content(
    base_url: *const c_char,
    access_token: *const c_char,
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_plugin_catalog(
    base_url: *const c_char,
    access_token: *const c_char,
) -> *mut c_char {
    let (Some(base), Some(token)) = (rstr(base_url), rstr(access_token)) else {
...
/// 引数は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_api_library(
    base_url: *const c_char,
    access_token: *const c_char,
) -> *mut c_char {
    let (Some(base), Some(token)) = (rstr(base_url), rstr(access_token)) else {
...
/// `root` / `meeting_id` は有効な NUL 終端 UTF-8。
#[no_mangle]
pub unsafe extern "C" fn genie_core_session_start(
    root: *const c_char,
    meeting_id: *const c_char,
) -> *mut CApiSession {
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_session_push(
...
/// `session` は生存ハンドル。
#[no_mangle]
pub unsafe extern "C" fn genie_core_session_recorded_ms(session: *mut CApiSession) -> u64 {
...
/// `session` は生存ハンドル。
#[no_mangle]
pub unsafe extern "C" fn genie_core_session_finish(session: *mut CApiSession) -> i32 {
    let Some(session) = session.as_mut() else {
        return -1;
    };
...
#[no_mangle]
pub unsafe extern "C" fn genie_core_session_free(session: *mut CApiSession) {
    if !session.is_null() {
