...
/// 既定の保存先（Application Support/Astra/meetings: retain recordings across the Genie rename）。OS の data dir を使う。
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("genie-core-{name}-{}", std::process::id()));
    ...
    fn snapshot_says_offline_while_the_link_is_down_but_stays_recording() {
        ...
        assert_eq!(snap.mode, GenieMode::Recording);
    ...
    fn snapshot_paused_is_not_offline() {
        ...
        assert_eq!(snap.mode, GenieMode::RecordingPaused);
