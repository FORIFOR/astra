...
//
//   occupation      Genie の窓が画面のどれだけを覆っているか
//   occlusion       前面アプリの窓を Genie がどれだけ隠しているか
//   frontmost       いま前面にいるアプリ（焦点を奪ったかの判定に使う）
//   astraFrontmost  Genie が前面か
//
...
func isGenie(_ w: [String: Any]) -> Bool {
    let o = owner(w); return o.contains("Genie") || o == "GenieMac"
}

// Genie の占有面積（窓どうしが重なっても二重に数えない）。
var astraRects: [CGRect] = []
for w in list where isGenie(w) {
...
for w in list where owner(w) == front && !isGenie(w) {
...
// **上から覆われている割合。** Genie が画面に在っても、他の窓が上に重なって
// いれば利用者には見えない。実際、待機中の HUD が macOS の再生ウィジェットに
// 完全に隠れていたことがある（それで「気付けない」が起きる）。
// CGWindowList は前から後ろの順に返るので、Genie より前にある窓だけを見る。
var covered: CGFloat = 0, astraArea: CGFloat = 0
if let first = list.firstIndex(where: { isGenie($0) }) {
    let above = list[0..<first].compactMap { w -> CGRect? in
        guard !isGenie(w), let r = rect(w), r.width > 8, r.height > 8 else { return nil }
        return r
    }
    for a in astraRects {
...
if let first = list.firstIndex(where: { isGenie($0) }) {
    for w in list[0..<first] {
        guard !isGenie(w), let r = rect(w) else { continue }
...
print("astraFrontmost=\(front.contains("Genie"))")
